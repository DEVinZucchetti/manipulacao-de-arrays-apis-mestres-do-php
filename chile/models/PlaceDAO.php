...
class PlaceDAO extends Database{
    ...
    public function updateOne($id, $data){
        ...
        $statement->bindValue(":contact_value",
                                isset($data->contact)?
                                $data-> contact :
                                $placeInDatabase['contact']
                            );
        $statement->bindValue(":opening_hours_value",
                                isset($data->opening_hours)?
                                $data-> opening_hours :
                                $placeInDatabase['opening_hours']
                            );
        $statement->bindValue(":description_value",
                                isset($data->description)?
                                $data-> description :
                                $placeInDatabase['description']
                            );
        $statement->bindValue(":latitude_value",
                                isset($data->latitude)?
                                $data-> latitude :
                                $placeInDatabase['latitude']
                            );
        $statement->bindValue(":longitude_value",
